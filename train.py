...
stale = 0
best_bleu = 0.0
_exp_name = "bleu_warmup_GA_convert_tfm"
...
for epoch in range(num_epochs):
    ...
    if valid_bleu > best_bleu:
        with open(f"./log/{_exp_name}_bs{bs}_log.txt","a") as f:
            f.write(f"[ Valid | {epoch + 1:03d}/{num_epochs:03d} ] bleu = {valid_bleu:.5f}, CIDEr = {total_score['CIDEr']:.5f}, RougeL = {total_score['ROUGE_L']:.5f} -> best\n")
    else:
        with open(f"./log/{_exp_name}_bs{bs}_log.txt","a") as f:
            f.write(f"[ Valid | {epoch + 1:03d}/{num_epochs:03d} ] bleu = {valid_bleu:.5f}, CIDEr = {total_score['CIDEr']:.5f}, RougeL = {total_score['ROUGE_L']:.5f}\n")
    # save models
    if valid_bleu > best_bleu:
